...
Now we will run HoldFst on whole genome data to determine significant regions across the genome. The whole genome data is not currently available due to its size, but there are publically available VCFs from already published studies that can be used to run HoldFst. 
```{r}
results<-HoldFst("~/Documents/package/Sample.vcf","Akagera","AkageraV",bin=500000,plot=T)
